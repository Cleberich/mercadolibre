...
export default function Detalles() {
  ...
  return (
    ...
    <p className='xs-1 mr-2'>{detalle?.available_quantity} vendidos</p>
      ...
      <div className=''>
        <img className="w-80 mx-auto" src={thumbnail} alt={title}/>
      </div>
      <div className="pl-3">
      <p className='line-through xs-1 text-gray-500'>{formatearDinero(price * 1.12)}</p>
      <p className='text-2xl my-auto'>{formatearDinero(price)} <span className='text-verde font-bold text-sm pb-1'>12% OFF</span></p>
      <p className='text-md'>en <span className='text-verde'>12 x {formatearDinero(price/12)} sin interés</span></p>
      ...
      <div className='bg-gray-100 w-90 rounded-md mx-auto py-3'>
        <p className='text-xs pl-3'>Cantidad: <b>1</b> <span className='ml-6 text-gray-500'>({detalle[0].installments.quantity} disponibles)</span></p>
      </div>
      <div className='mt-3'>
