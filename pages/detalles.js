...
export default function Detalles() {
  ...
  return (
    ...
    <p className='xs-1 mr-2'>{detalle?.available_quantity} vendidos</p>
      ...
      <div className="pl-3">
      <p className='line-through xs-1 text-gray-500'>{formatearDinero(detalle ? detalle?.price * 1.12 : (323))}</p>
      <p className='text-2xl my-auto'>{formatearDinero(detalle ? detalle?.price : (213))} <span className='text-verde font-bold text-sm pb-1'>12% OFF</span></p>
      <p className='text-md'>en <span className='text-verde'>12 x {formatearDinero(detalle ? detalle?.price/12 : (233))} sin interés</span></p>
